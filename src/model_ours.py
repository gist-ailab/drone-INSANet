...
class VGGEdited(nn.Module):
    ...
    """
    def __init__(self, attention =['CBAM', 'CHANNEL', 'SPATIAL'], pos = ['first', 'last', 'all'], fusion = ['add', 'cat']):
    ...
    def GetPostAttention(self, channel):
        if self.fusion == 'add':
            ...
            channel = channel * 2
        if self.attention == 'CBAM':
            att= CBAM(channel).to(device)
        elif self.attention == 'CHANNEL':
            att= ChannelGate(channel).to(device)
        elif self.attention == 'SPATIAL':
            att=SpatialGate().to(device)
            channel = channel
        else:
            print(self.fusion)
        return att, channel
    ...
    def forward(self, image_vis, image_lwir):
        ...
        if self.pos == 'first' or self.pos == 'all':
            out_lwir_1 = self.pre_att_ir(out_lwir_1)
...
class ATTNet(nn.Module):
    ...
    def __init__(self, n_classes, attention =['CBAM', 'CHANNEL', 'SPATIAL'], pos = ['first', 'last', 'all'], fusion = ['add','cat']):
        super(ATTNet, self).__init__()

        self.n_classes = n_classes
        self.base = VGGEdited(attention, pos, fusion)
    ...
    def forward(self, image_vis, image_lwir):
        ...
        :param image: images, a tensor of dimensions (N, 3, 512, 640), (N, 1, 512, 640)
        :return: 41760 locations and class scores (i.e. w.r.t each prior box) for each image
        """
        # Run INSA network convolutions (feature extractor)
        ...
        locs, classes_scores = self.pred_convs(conv4_3_feats, conv6_feats, conv7_feats, conv8_feats, conv9_feats, conv10_feats) # (N, 41760, 4), (N, 41760, n_classes)

        return locs, classes_scores

    def create_prior_boxes(self):
        """
        Create the 41760 prior boxes for the network, as defined in the paper.

        :return: prior boxes in center-size coordinates, a tensor of dimensions (41760, 4)
        """
                    ...
                    for s in anchor_areas[fmap]:
                        for ar in aspect_ratios[fmap]: 
                            ...
                            for sr in scale_ratios[fmap]: # scale
                                anchor_h = h*sr/512. # height scale (512)
                                anchor_w = w*sr/640. # width scale (640)
                                prior_boxes.append([cx, cy, anchor_w, anchor_h])

        prior_boxes = torch.FloatTensor(prior_boxes).to(device)  # (number of , 4)

        return prior_boxes

    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap, top_k):
        """
        Decipher the 41760 locations and class scores (output of ths INSA) to detect objects.

        For each class, perform Non-Maximum Suppression (NMS) on boxes that are above a minimum threshold.

        :param predicted_locs: predicted locations/boxes w.r.t the 41760 prior boxes, a tensor of dimensions (N, 41760, 4)
        :param predicted_scores: class scores for each of the encoded locations/boxes, a tensor of dimensions (N, 41760, n_classes)
        ...
        return all_images_boxes, all_images_labels, all_images_scores, all_images_bg_scores
...
class MultiBoxLoss(nn.Module):
    ...
    (1) a confidence loss for the predicted class scores, and
    (2) a localization loss for the predicted locations of the boxes.
    """
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        :return: (multibox loss, confidence loss, localization loss, number of positives)
        """
        ...
        for i in range(batch_size):
            ...
            true_locs[i] = cxcy_to_gcxgcy(xy_to_cxcy(boxes[i][object_for_each_prior]), self.priors_cxcy)  # (41760, 4)

        # Identify priors that are positive (object/non-background)
        positive_priors = true_classes > 0  # (N, 41760)
        ...
        # Note: indexing with a torch.uint8 (byte) tensor flattens the tensor when indexing is across multiple dimensions (N & 41760)
        # So, if predicted_locs has the shape (N, 41760, 4), predicted_locs[positive_priors] will have (total positives, 4)
        ...
        conf_loss_all = conf_loss_all.view(batch_size, n_priors)  # (N, 41760)
        ...
        conf_loss_neg = conf_loss_all.clone()  # (N, 41760)
        conf_loss_neg[positive_priors] = 0.  # (N, 41760), positive priors are ignored (never in top n_hard_negatives)
        conf_loss_neg, _ = conf_loss_neg.sort(dim=1, descending=True)  # (N, 41760), sorted by decreasing hardness
