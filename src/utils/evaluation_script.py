    ...
    def accumulate(self, p=None):
        ...
        for k, k0 in enumerate(k_list):
            ...
            for m, maxDet in enumerate(m_list):
                ...
                # sort dtScore by highest to index from 0
...
def draw_all(eval_results, save_path, filename='figure.jpg'):
    ...
    MR_day = [res['day'].summarize(0) * 100 for res in eval_results][0]
    MR_night = [res['night'].summarize(0) * 100 for res in eval_results][0]
    recall_all = (1 - eval_results[0]['all'].eval['yy'][0][-1]) * 100
    
    txtfile = open(os.path.join(save_path, 'results.txt'), 'a+')
    txtfile.write(f'\n{methods[0]}: MR_all: {MR_all:.2f}, MR_day: {MR_day:.2f}, MR_night: {MR_night:.2f}, Recall:{recall_all:.2f}')
    txtfile.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--jobsDir', type=str, default=None,
                        help='Please put the path of the parent path (jobs) where the result file is located.')
    parser.add_argument('--rstFiles', type=str, nargs='+', default=None,
    ...
    if args.rstFiles is not None:
        for rstFile in args.rstFiles:
            ...
            if not os.path.isdir(savePath):
                os.mkdir(savePath)
            draw_all(results, savePath, filename=rstFile.rsplit('/')[-1].replace('.txt', '.jpg'))
    else:
        jobs_files = os.listdir(args.jobsDir)
        ...
        for rstFile in rstFiles:
            ...
            if not os.path.isdir(savePath):
                os.mkdir(savePath)
            draw_all(results, savePath, filename=rstFile.replace('.txt', '.jpg'))
