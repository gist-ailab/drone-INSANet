...
def box_translate(box, t, width):
    ...
    inner_box = [box[0]]
    for box_ in box[1:]:
        xmin, ymin, xmax, ymax, pair = box_
        if xmin >= 0.0 and xmax <= 1.0:
            inner_box.append([xmin, ymin, xmax, ymax, pair])
    inner_box = np.array(inner_box)
        
    return inner_box


def box_translate_xy(box, t_x, t_y, width, height):
    if not _is_numpy_box(box):
        pdb.set_trace()
        raise TypeError('box is not a numpy array.')
    ...
    for box_ in box[1:]:
        xmin, ymin, xmax, ymax, pair = box_
        if xmin >= 0.0 and xmax <= 1.0 and ymin >= 0.0 and ymax <= 1.0:
            inner_box.append([xmin, ymin, xmax, ymax, pair])
    inner_box = np.array(inner_box)
